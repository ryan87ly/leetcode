...
public class Solution2 {
    ...
    public List<List<String>> findLadders(String beginWord, String endWord, Set<String> wordList) {
        ...
        while(!queue.isEmpty()) {
            ...
            LinkedList<Integer> sequence = sp.sequence;
            ...
            if (currentWord.adjacentNodes.contains(1)) {
                List<String> r = convertToList(sequence);
                r.add(endWord);
                result.add(r);
            ...
            for (int i : currentWord.adjacentNodes) {
                ...
                if (accessed == null || accessed.intValue() >= currentLevel + 1) {
                    ...
                    LinkedList<Integer> copied = new LinkedList<>(sequence);
                    copied.add(i);
    ...
    /*private void search(List<List<String>> result, int currentIndex, LinkedHashSet<Integer> sequence, String endWord) {
        if (sequence.size() > minWords) return;
        Node currentWord = words[currentIndex];
        if (currentWord.adjacentNodes.contains(1)) {
            List<String> r = convertToList(sequence);
            r.add(endWord);
        ...
        for (int i : currentWord.adjacentNodes) {
            if (!sequence.contains(i)) {
                LinkedHashSet<Integer> copied = new LinkedHashSet<>(sequence);
                copied.add(i);
                search(result, i, copied, endWord);
            }
        }
    }*/

    private List<String> convertToList(LinkedList<Integer> sequence) {
    ...
    private static class SearchPoint {
        ...
        private SearchPoint(int index, LinkedList<Integer> sequence) {
            this.index = index;
